...
quote_token = "0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48"          #[add second token for pool]
index_pool = "1"                                                    #[add index for pool to check (0 - 3)]
reqular_quote_timeframe = "1800"                                    #[add timeframe in seconds]
...
def handle_event(blockNumber):
    ...
    Table_namesSubmit_changes = ["BLOCKNUMBER", "CURRENT INDEX", "TIMESTAMP", "TICK CUMULATIVE", "SECONDS LIQUIDITY CUMULATIVE", "INITIALIZED", "CONSULT_CUMULATIVE_0", "CONSULT_CUMULATIVE_1", "CONSULT_WEIGHT_0","CONSULT_WEIGHT_1"]
     #Store CSV Table 
    ...
    while loop > 0:
        ...
        if loop == checks:
            ...
            Table_namesSubmit = ["INDEX", "TIMESTAMP", "TICK CUMULATIVE", "SECONDS LIQUIDITY CUMULATIVE", "INITIALIZED", "CONSULT_CUMULATIVE_0", "CONSULT_CUMULATIVE_1", "CONSULT_WEIGHT_0","CONSULT_WEIGHT_1"]
            #Store CSV Table 
            with open(Path_storeCSV_first, 'a') as writeFile:
                writer = csv.writer(writeFile)
            ...
            while x < pool_slot0[3]:
                consult = contracts_pools[index_pool].functions.observe([reqular_quote_timeframe, 0]).call(block_identifier = checkBlock)
                observation = contracts_pools[index_pool].functions.observations(x).call(block_identifier = checkBlock)
                array_observations = []
                array_observations.extend([x, observation[0], observation[1], observation[2], observation[3], consult[0][0], consult[0][1], consult[1][0], consult[1][1]])
                # Add data to CSV file
                newRow = []
            ...
            if pool_slot0[2] != last_index:
                ...
                # Add data to CSV file
                newRow = []
                newRow.extend([checkBlock, pool_slot0[2], observation[0], observation[1], observation[2], observation[3], consult[0][0], consult[0][1], consult[1][0], consult[1][1]])
